...
function [pin_blcko,pin_block,type_block,xyblock]=gen_ess_blocks_L(thw,kgraph,Nx,Col,Row,Rowdist,z,ne,T,S,a,GD) 
...
if a==1
   ...
   load wire_quad_mesh; x1=xx; y1=yy; t1=tt; sn1=ssl;
   load wire_quad_mesh_flat; x2=xx; y2=yy; t2=tt; s2=ssl;
else
   
    if T>0
        load wire_quad_mesh_flat; x=xx; y=yy; t=tt; sn=ssl; 
    else
        load wire_quad_mesh; x=xx; y=yy; t=tt; sn=ssl; 
    end
end
...
for e=1:nel; c1=ve(1,e);c4=ve(4,e);pin=qe(e);
  ...
  if flag(c1,c4)==0; nedge=nedge+1;flag(c1,c4)=nedge;
    if pin>0;
      [xedge(1:m,nedge),yedge(1:m,nedge)]=xy_pin(c1,c4,e,pin,Xsl,Ysl,X,Y,w,m,T,x,y,t,sn);
...
for e=1:nel;    
    % Adjust
   % Find weight of flat mesh vs regular mesh templates
   if a==1
      pf=(gaps(e)-1)/3; 
      if pf<1
          x=pf*x1+(1-pf)*x2;
   ...
   if pin>0 && cpin(pin)>0 
    ee=ee+1; ee2e(ee) = e; e2ee(e) = ee;
      ...
      for i=1:medge; 
         ...
         [xx,yy,tp]=xy_pin2(pin,xedge(i,kedge),yedge(i,kedge),X,Y,w,m2,distR,x,y,t,sn);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function grd=pin_grad(x,xc,yc,wire,pin) %10


h=1.e-5;
px = [ x(1)-h , x(1)+h ,  x(1)   , x(1)   ]'; 
py = [ x(2)   , x(2)   ,  x(2)-h , x(2)+h ]'; 
X = [ px , py ]; Z = fdpnwr(X,xc,yc,wire,pin);
grd=0*x; grd(1)=.5*(Z(2)-Z(1))/h; grd(2)=.5*(Z(4)-Z(3))/h;


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function d=fdpnwr(p,xc,yc,wire,pin) %11


  R = wire(4);  % Pin radius
  d=fdpins(p,xc,yc,R); dw=fdwire(p,xc,yc,wire,pin); d=min(d,dw);
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function d=fdwire(p,xc,yc,wire,pin) %13
...
for k=1:nc;
  ...
  if (T>0)
    ...
    these=(dist.*cos(th2)>(line_x-2*Tw) & abs(dist.*sin(th2))<(rw-Tw)*tan(acos((rw-Tw)/rw)));
...
for k=1:maxiter;
   ...
   dw=fdwire(PP,X(cellp),Y(cellp),wire,cellp);
...
if (th>th1 && th<th2 && lsf<0.9) %|| (thn>th1 && thn<th2)
ls=dX*lsf;
...
%plot(xth(1),xth(2),'m*')
    ...
    dbef=di/d12;
    daft=1-di/d12;
    nbef=round((m-1)*dbef^(3/4)/(dbef^(3/4)+daft^(3/4)));
    naft=round((m-1)*daft^(3/4)/(dbef^(3/4)+daft^(3/4)));
    
    dbef=dbef*d12;
    daft=daft*d12;
  
    while nbef*ls>dbef && nbef>1 && (nbef-1)*dX>dbef
        nbef=nbef-1;
        naft=naft+1;
    end
    
    while naft*ls>daft && naft>1 && (naft-1)*dX>daft
        naft=naft-1;
        nbef=nbef+1;
    end
...
%     pcc1,pause
      ...
      dw=fdwire(PP,X(pcc1),Y(pcc1),wire,pcc1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
function [xedge,yedge]=xy_pin(c1,c4,e,pin,Xcl,Ycl,X,Y,wire,m,T,xx,yy,tt,ssl) %17
...
x1=x0;
h=fdpnwr(x1,X(pin),Y(pin),wire,pin); grd=pin_grad(x1,X(pin),Y(pin),wire,pin);
iter=0;
while abs(h) > 100*eps && iter < 30; iter=iter+1;
   x0=x1; x1=x0-h*grd/norm(grd); h=fdpnwr(x1,X(pin),Y(pin),wire,pin);
%  [x1, iter, h]
end;
x=x1;
...
grd=pin_grad(x0,X(pin),Y(pin),wire,pin);
x1 = x0+h*grd/norm(grd); d=fdpnwr(x1,X(pin),Y(pin),wire,pin);
...
while norm(F) > 100*eps && iter < 200; iter=iter+1;
   S0=S1;
%  norm(F), iter   %   , toc
   ...
   if norm(F) > 100*eps && iter>30; S1=S1-.4*(J\F);end;
...
%      if t(i)<pi/8 && k==12; 
%          t(i)=2*pi; 
%      end; % Make sure right corner pin's, top block is first
  end;
    if k==12
      t(5)=2*pi;
  end
...
function [nvtx,nevtx,ntcl,tcell,necl,ecell,nccl,ccell,X,Y]=...
    get_cells(ne,npin,km,xm,ym,X,Y,D,P,G) % Construct cells connecting pins %36
