    function build_wire_template_L(PD,Dw,Df,T,S)
%
...
%%    HEDL:                    1.2435    ( = .286/.23)
      ...
      triangulate_wire_template(PD,Dw,Df,T,S); %Uncomment This!
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function triangulate_wire_template(PD,Dw,Df,TD,SD)
...
T = TD*D;               % Trim off top of wire
S = SD*D;               % Submersion of wire into pin
...
wire=[P,R;rw,rf;Db,Xb;thw,thb;fix;T,S];
...
for k=1:nc;
  ...
  %%%%%%%%%%%%%%%%%%%%%%%% Modified here Landon Flattening
...
%ms = 8;
ms = 16; xo=[po,u2]; ssl=[0:ms]'/ms; xx=zeros(ms+1,m+1);yy=xx;


%%%%Landon new ssl: more circumferential lines toward the inside

ratio=0.8;
ssl=[0:ms]'.^(ratio);
%ssle=ssl/ssl(end)
%ssl(ms+1)=ssl(ms+1)-(ssl(ms+1)-ssl(ms))/2; % Makes first gap smaller
ssl=ssl/ssl(ms+1);
...
ratio= 1.035; % Ratio for geometric series shifting inside distribution
power= 1.5; % Power used to weight the combination of inside and outside when merging meshes
...
% Move distribution toward fillets
%

for i=ns-1:-1:2
    gap=((xt(i+1,:)-xt(i,:)).^2+(yt(i+1,:)-yt(i,:)).^2).^(1/2);
    sg=min(gap);lg=max(gap);
    move=(i)/(ns-1)*lg/(lg-sg)*(gap/lg-sg/lg)*0.5;
    xt(i,:)=(1-move).*xt(i,:)+move.*xt(i+1,:);
    yt(i,:)=(1-move).*yt(i,:)+move.*yt(i+1,:);    
end
%}
...
figure
hold off
mesh(xt,yt,0*xt,'EdgeColor','blue'); view(0,90); axis image; axis off;
