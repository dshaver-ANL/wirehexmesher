...
%      trimesh(t,p(:,1),p(:,2),zeros(N,1))
  ...
  dgradx=(feval(fd,[p(ix,1)+deps,p(ix,2)],varargin{:})-d(ix,1))/deps; % Numerical
  dgrady=(feval(fd,[p(ix,1),p(ix,2)+deps],varargin{:})-d(ix,1))/deps; %    gradient
  p(ix,:)=p(ix,:)-[d(ix,1).*dgradx,d(ix,1).*dgrady];     % Project back to boundary

  % 8. Termination criterion: All interior nodes move less than dptol (scaled)
  if max(sqrt(sum(deltat*Ftot(d<-geps,:).^2,2))/h0)<dptol, break; end
% if k>25000, break; end
% find(t==5)
% if k>1500, break; end
% if k>500, break; end
  if k>20, break; end
...
function trimeshpf(t,x,y)
n=length(x);
E=size(t,1);
e=1; tt=[ t(e,:) , t(e,1) ];
close all
hold off; plot(x(tt),y(tt),'g-'); hold on
for e=1:E
  tt=[ t(e,:) , t(e,1) ]; plot(x(tt),y(tt),'g-');
end;
